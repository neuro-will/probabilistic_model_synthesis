...
from probabilistic_model_synthesis.simulation import SqFcn
...
ps['n_ex_systems'] = 10  # Use >= 100 for publication
...
if ps['sim_type'] == 'props': 
    ps['n_smps_range'] = [150000, 200000] 
elif ps['sim_type'] == 'no_props':
    # Because we have less structure regularization when not using properties, we need many more samples
    ps['n_smps_range'] = [150000, 200000] #[100000, 110000]
...
if ps['sim_type'] == 'props':
    ...
    ps['fit_w_prior_opts'] = {'mn_hc_params': fit_hc_params, 'std_hc_params': fit_hc_params,
                              'min_std': .000001, 'mn_init': 0.0, 'std_init': 300} 
...
ps['w_post_opts'] = {'mn_mn':0.0, 'mn_std': .0001, 'std_iv': .0001}

# Options for the densenet which makes up the shared-m module
ps['dense_net_opts'] = {'n_layers': 2, 'growth_rate': 150, 'bias': True}
...
if ps['sim_type'] == 'props':
    ...
    ps['single_sp_fit_opts'] = [{'n_epochs': 1, 'milestones': None, 'update_int': 100, 'init_lr': .0, 'n_batches': 2}]
    ps['single_ip_fit_opts'] = [{'n_epochs': 1000, 'milestones': [1000], 'update_int': 100, 'init_lr': .001, 'n_batches': 2},
                                {'n_epochs': 1000, 'milestones': [1000], 'update_int': 100, 'init_lr': .001, 'n_batches': 2}]
...
ps['save_folder'] = r'/groups/bishop/bishoplab/projects/probabilistic_model_synthesis/results/simulation/gnlr/no_props/with_props'
...
min_proj_vl = -1.96*np.sqrt(ps['n_input_var_range'][0])
max_proj_vl = 1.96*np.sqrt(ps['n_input_var_range'][0])
...
comb_fit_rs = fit_with_hypercube_priors(data=ind_data, props=ind_props, p=ps['p'],
                                        ...
                                        match_init_ip_w_post_to_prior=ps['sim_type'] == 'props')
