...
if __name__ == '__main__':
    ...
    if not load_previous_results:
        ...
        print('Loading previous simulation results from ' + str(SAVE_PATH))
    ...
    plt.ylim(0, 2.5)
    plt.xlabel('Number of example systems', fontsize=8)
    plt.ylabel('RMSE', fontsize=8)
    plt.legend(['RMSE'], fontsize=8)
    plt.title('Recovery of mean', fontsize=8)
    ...
    plt.ylim(0, 1.5)
    plt.xlabel('Number of example systems', fontsize=8)
    plt.ylabel('Geometric mean / RMSE', fontsize=8)
    plt.title('Recovery of variances', fontsize=8)
    plt.legend(['RMSE', 'Geometric mean'], fontsize=8)
    plt.show()

    fig.savefig(FIG_SAVE_PATH, format='svg', transparent=True)
